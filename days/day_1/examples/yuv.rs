...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for y in 0..even_h {
        for x in 0..even_w {
            ...
            bright_mask.put_pixel(x, y, Luma([if yy > 170 { 255 } else { 0 }]));
        }
    }
    ...
    win_rgb.set_image(
        "rgb",
        ImageView::new(ImageInfo::rgb8(even_w, even_h), rgb.as_raw()),
    )?;
    win_recon.set_image(
        "recon",
        ImageView::new(ImageInfo::rgb8(even_w, even_h), recon.as_raw()),
    )?;
    win_y.set_image(
        "y",
        ImageView::new(ImageInfo::mono8(even_w, even_h), y_img.as_raw()),
    )?;
    win_mask.set_image(
        "mask",
        ImageView::new(ImageInfo::mono8(even_w, even_h), bright_mask.as_raw()),
    )?;
