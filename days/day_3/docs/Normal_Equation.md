...
Cách gộp:
- Mỗi dòng `x_i = [x1, x2, ..., xd]`
- Thêm cột 1 đầu dòng: `xbar_i = [1, x1, x2, ..., xd]`
- Khi đó:
...
Nhược:
- Phải tính nghịch đảo ma trận \((d+1)\times(d+1)\), tốn chi phí \(O(d^3)\)
...
- kiểm tra định thức gần 0 -> báo lỗi
...
(Cái này gọi là closed-form cho Ridge Regression)
...
8. `predict_batch(X, w_full) -> y_hat`
9. `mse_loss(y, y_hat)`
...
Lưu ý:
- predict cần tự thêm bias logic:
...
- `add_bias_column`
  - Dùng công thức mở rộng vector đặc trưng: \(\bar{x}=[1, x_1,...,x_d]\)
